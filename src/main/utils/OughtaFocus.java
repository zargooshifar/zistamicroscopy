...
//            channel = getPropertyValue(CHANNEL);
    ...
    public double fullFocus() throws MMException {
        ...
        try {
            ...
            liveModeOn_ = Singletons.getCameraControllerInstance().isLive();
...
//      getCurrentFocusScore();
            ...
            if (liveModeOn_) {
                ...
                if (show.contentEquals("Yes")) {
                    SwingUtilities.invokeLater(new Runnable() {
                        ...
                        @Override
                        public void run() {
                            Singletons.getCameraControllerInstance().getCanvas().setTaggedImage(img1);
                            Singletons.getCameraControllerInstance().getCanvas().repaint();
                        }
                    });
                }
            }
    ...
    public double getCurrentFocusScore() {
        ...
        try {
            ...
            if (show.contentEquals("Yes")) {
                Singletons.getCameraControllerInstance().getCanvas().setTaggedImage(img);
                Singletons.getCameraControllerInstance().getCanvas().repaint();
            }
